...
jQuery(document).ready(function ($) {
    $('.cpool').click(function (event) {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax/projectAjax.php?cmd=3&type='+type+'&dest='+dest+'&capacity='+capacity+'&time='+time+'&username='+cpool,
            error: function () {
                // alert('error, failed to get id');
            },
            success: function (html) {
...
jQuery(document).ready(function ($) {
    $('.jPool').click(function (event) {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax/projectAjax.php?cmd=4',
            error: function () {
                // alert('error, failed to get id');
            },
            dataType: 'json',
...
function dialog(i,payment)
{
    $.ajax({
        type: 'POST',
        url: 'ajax/projectAjax.php?cmd=5&oid='+array[i].driver+'&cid='+array[i].carID+'&pay='+payment+'&pid='+sessionStorage.id+'&count='+array[i].full
...
jQuery(document).ready(function ($) {
    $('.pJoin').click(function (event) {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax/projectAjax.php?cmd=6&id='+sessionStorage.id,
            error: function () {
                // alert('error, failed to get id');
            },
            dataType: 'json',
...
function onSuccess(position) {
    ...
    $.ajax({
        type: 'POST',
        url: 'ajax/projectAjax.php?cmd=7&id='+sessionStorage.id+"&lat="+lat+"&long="+long,
        error: function () {
            // alert('error, failed to get id');
        },
        dataType: 'json',
...
jQuery(document).ready(function ($) {
    $('.add').click(function (event) {
        $.ajax({
            type: 'POST',
            url: 'ajax/projectAjax.php?cmd=8&id='+sessionStorage.id+'&message='+$(".info").val()+'&pic='+sessionStorage.image,
            error: function () {
                // alert('error, failed to get id');
            },
            dataType: 'json',
